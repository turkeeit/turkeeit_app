const connection = require("../config/dbconfig");

function editPartnerDetails(req, res) {
  console.log("Editing partner details...");

  const partner_id = req.headers.mobile_number;

  const {
    name,
    gender,
    service_category_id,
    experience,
    flat_no,
    building_name,
    area_name,
    landmark,
    city,
    state,
    pincode,
  } = req.body;

  if (
    !partner_id ||
    !name ||
    !gender ||
    !service_category_id ||
    experience === undefined ||
    !flat_no ||
    !building_name ||
    !area_name ||
    !landmark ||
    !city ||
    !state ||
    !pincode
  ) {
    return res.status(400).json({
      error:
        "mobile_number, name, gender, service_category_id, experience, flat_no, building_name, area_name, landmark, city, state and pincode are required",
    });
  }

  const updatePartnerQuery = `
    UPDATE partners
    SET
      name = ?,
      gender = ?,
      service_category_id = ?,
      experience = ?
    WHERE mobile_number = ?
  `;

  connection.query(
    updatePartnerQuery,
    [name, gender, service_category_id, experience, partner_id],
    (partnerErr, partnerResult) => {
      if (partnerErr) {
        console.error("Error updating partner:", partnerErr);
        return res.status(500).json({ error: "Error updating partner" });
      }

      if (partnerResult.affectedRows === 0) {
        return res.status(404).json({ error: "Partner not found" });
      }

      const updateAddressQuery = `
        UPDATE addresses
        SET
          flat_no = ?,
          building_name = ?,
          area_name = ?,
          landmark = ?,
          city = ?,
          state = ?,
          pincode = ?
        WHERE user_id = ?
      `;

      connection.query(
        updateAddressQuery,
        [
          flat_no,
          building_name,
          area_name,
          landmark,
          city,
          state,
          pincode,
          partner_id,
        ],
        (addressErr, addressResult) => {
          if (addressErr) {
            console.error("Error updating address:", addressErr);
            return res.status(500).json({ error: "Error updating address" });
          }

          if (addressResult.affectedRows === 0) {
            return res.status(404).json({ error: "Address not found" });
          }

          return res.status(200).json({
            message: "Partner details updated successfully",
          });
        },
      );
    },
  );
}

module.exports = editPartnerDetails;
